/* ==========================================
   STACKLY EVENTS — auth.js
   localStorage-based auth system
   ========================================== */

const AUTH_KEY = 'stackly_events_user';
      ...
      loginWrap.innerHTML = `
        <div class="nav-user-chip" id="nav-user-chip" role="button" aria-label="User menu" aria-expanded="false" tabindex="0">
          ...
          <div class="nav-user-dropdown" id="nav-user-dropdown" role="menu">
            ...
            <a href="events.html" role="menuitem">
  ...
  if (mobileLoginWrap) {
    if (user) {
      ...
      mobileLoginWrap.innerHTML = `
        <div style="display:flex;align-items:center;gap:0.75rem;padding:0.85rem 1rem;background:rgba(255,255,255,0.04);border-radius:18px;border:1px solid var(--border);">
          <div style="width:38px;height:38px;border-radius:50%;background:linear-gradient(135deg,var(--primary),var(--secondary));display:flex;align-items:center;justify-content:center;font-weight:800;color:var(--bg-main);font-size:1rem;flex-shrink:0;">${initial}</div>
      ...
      mobileLoginWrap.innerHTML = `<a href="login.html" style="display:flex;align-items:center;justify-content:center;padding:0.8rem;border-radius:999px;background:var(--primary);color:var(--bg-main);font-weight:700;font-size:0.9rem;">Login to Your Account</a>`;
    }
  }
}
